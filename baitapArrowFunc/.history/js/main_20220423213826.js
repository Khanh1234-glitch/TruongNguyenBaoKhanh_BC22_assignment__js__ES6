...
for (let value of colorList) {
    ...
    const fn = () => {
        let button1 = "<button onclick='changeColor()' class= 'color-button pallet active '></button>";
        ...
        document.getElementById("colorContainer").innerHTML = button1 + button2 + button3 + button4 + button8 + button9 + button10 + button11 + button12;
    }
    fn();
}
const changeColor = () => {
    document.getElementsByClassName("pallet")[0].addEventListener("click", () => {
        var result = document.getElementById('house');
        result.classList.toggleclass('pallet');
    })
}